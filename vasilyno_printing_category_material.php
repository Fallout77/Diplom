...
$price_list = VASILYNO_get_category_material();
...
$active_sheet->setTitle("Категории");	
...
$active_sheet->setCellValue('A2','Управление категориями');

$active_sheet->setCellValue('A6','№п.п');
$active_sheet->setCellValue('B6','Категория');
...
foreach($price_list as $item) {
	...
	$active_sheet->setCellValue('B'.$row_next,$item['name_material_category']);
...
$active_sheet->getStyle('A1:B'.($i+6))->applyFromArray($style_wrap);
...
$active_sheet->getStyle('A1:B1')->applyFromArray($style_header);
...
$active_sheet->getStyle('A2:B2')->applyFromArray($style_slogan);
...
$active_sheet->getStyle('B4')->applyFromArray($style_date);
...
$active_sheet->getStyle('A6:B6')->applyFromArray($style_hprice);
...
header("Content-Disposition:attachment;filename='category_material.xls'");
