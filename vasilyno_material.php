...
if ($_POST["action"])
{
	...
	//...................................................................
	// *** Добавление или редактирование
	$query = "INSERT INTO `{$VASILYNO['db_material_now']}` SET $sql_str ON DUPLICATE KEY UPDATE $sql_str";
	//print_pre ($query);
	$result = sql_query ($query);
	if (sql_aff_rows() > 0)
	{
		if (!$_POST['id_material']) $_POST['id_material'] = sql_last_id ();
		
		$warning = "Данные о материале #{$_POST['id_material']} [{$_POST['name']}] сохранены в БД";
...
if ($del)
{
	$query  = "DELETE FROM `{$VASILYNO['db_material_now']}` WHERE `id_material`='{$del}' LIMIT 1";
	$result = sql_query ($query);
	if (sql_aff_rows() > 0)
	{
...
// запрос в БД
$query  = "SELECT * FROM `{$VASILYNO['db_material_now']}` WHERE 1 $WHERE $SORT LIMIT $limit,$lines_on_page";
//print_pre ($query);
$result = sql_query ($query);
...
// сколько всего
$total = sql_get_num ($VASILYNO['db_material_now']);
...
<thead>
<tr>
	<th><a href="<?=$url_sort?>&order=id_material">#</a><? if ($order=="id_material") echo "▼"; ?></th>
...
<? foreach ($array_material_now as $material_now) : ?>
<tr>
	<td class="tbl_id"><?=$material_now['id_material']?></td>
	<td><a href="<?=$url?>.material_view.<?=$material_now['id_material']?>"><?=$material_now['name']?></a></td>
	<td><?=$material_now['price_product']?></td>
	<td><?=$material_now['price_per_gram_product']?></td>
	<td><?=$material_now['volume_in_storage']?></td>
	<td><?=$material_now['discription']?></td>
	<td class="tbl_admin">[<a href="<?=$url?>.material_form.<?=$material_now['id_material']?>">EDIT</a>]&nbsp;&nbsp;[<a href="#" onclick="_.confirm('Материал #<?=$material_now['id_material']?> `<?=$material_now['name']?>` будет удален. Продолжить?', '<?="{$url_page}&lines={$lines_on_page}&del={$material_now['id_material']}"?>')">DEL</a>]</td>
</tr>
<? endforeach ; ?>
